...
import createBufferFromReadStreamFile from 'server/infra/commons/utils/createReadStreamBuffer.utils';
...
export class UserResolver {
  ...
  public async createUser(
    ...
    const newFile: CreateFileDTO = await this.createFile(file);
  ...
  private async createFile(file: FileUpload): Promise<CreateFileDTO> {
    if (file) {
      const base64 = await createBufferFromReadStreamFile(file);
